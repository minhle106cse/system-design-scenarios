<!-- TEMPLATE — copy into <scenario>/directives/ and specialize.
     SPECIALIZE: the module name under `modules/`; which slots are ⏸ empty vs populated; the lint-boundary table if the scenario's eslint config differs.
...
4. Is this **scheduling arithmetic / a constraint rule**?
   - → the dependency-free core package — never a module folder
