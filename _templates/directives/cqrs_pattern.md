...
> reason worth reading before you assume this is churn: Cortex's own two directives had contradicted
> each other for ~6 weeks (`folder_structure_sop.md`'s canonical tree listed
> `application/repositories/` as the query-repo home while `cqrs_pattern.md` declared that folder
> banned), nothing cross-checked them, and the code followed the ban. The visible symptom was that
> `application/queries/` held two different KINDS of thing at once — per-query sub-folders
> (`get-schedule/`, each a `.query.ts` + `.handler.ts` pair) sitting next to loose port files
> (`scheduling.query-repository.ts`) — so a reader could not tell ports from use-cases without
> opening files. The original 2026-07-03 concern was never the folder itself but that it had **no
> defined meaning** and became an "I'm not sure where this goes" bucket. That is answered by
> definition, not deletion: it now holds exactly one thing.
...
|---|---|---|---|
| **Command / write-side** (entity-based, serves command handlers; mutation goes through a mapper when an Entity exists) | `domain/repositories/` | `<name>.repository.ts` | write-input types **inline** in the file (`CreateShiftData`, `AssignmentInput`, …) |
...
| **Domain READ port** (a **domain-layer class** — a domain service, not just a query handler — depends on it; domain must not import `application/`, so this is structural, not a style choice) | `domain/repositories/` | `<name>.repository.ts`, interface named `I{X}Reader` | **inline**; impl is `Prisma{X}ReaderRepository` / `<x>-reader.repository.ts`, **never** `.query-repository.ts` |
| **Application READ port** (only an `application/`-layer class consumes it — a query handler, or an application service — nothing in `domain/` imports it) | `application/repositories/` | `<module>.query-repository.ts` | response DTO in its own `<module>.dto.ts`, OR inline while there is exactly one query-repo and one DTO shape |
...
so step 2 alone would wrongly evict all of them. (Cortex measured this: 15 of its 19
...
2. **Read-only port. Does any file under `domain/` import it?**
   ...
   - **No → `application/repositories/`**, named `I{X}QueryRepository`, file
     `<module>.query-repository.ts`. `ISchedulingQueryRepository` is exactly this: consumed only by
     query handlers, nothing in `domain/` touches it.
...
- A query-repo interface shared by more than one query lives in **`application/repositories/`**,
  not buried inside one query's sub-folder — `ISchedulingQueryRepository` is exactly this: shared
  by `GetScheduleHandler`, `GetSummaryHandler`, and `GetCoverageHandler`.
- **Response DTOs stay in `application/queries/`** (they belong to the use-case, not the port) and
  ...
  shape (`ScheduleDetail`), so it still lives inline in the query-repo file — the moment a second
  query needs a different shape, split it out into `application/queries/scheduling.dto.ts`, and the
  port then imports it as `from '../queries/scheduling.dto'`.
