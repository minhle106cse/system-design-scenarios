<!-- TEMPLATE — copy into <scenario>/directives/ and specialize.
     SPECIALIZE: whether the domain has real state-machine entities (port Cortex/sc01's entity-factory + mutable-`_fields` rules in full) or plain-interface entities (trim, and say why).
...
Zero runtime dependencies, plain data in, plain data out. Consequences for modeling:
...
- The **one** exception is `FeasibilityGate`/`RosterState` (`assignment/feasibility-gate.ts`) —
  deliberately the single place mutation is allowed . Do not add a second mutable class
...
- Every entity under `domain/entities/` (`Schedule`, `StaffMember`, `Shift`, `DemandCell`,
  ...
  dependency-free core package, not here. A domain entity's whole job in this module is to
...
- A **repository** (`infrastructure/repositories/prisma-{entity}.repository.ts`,
  ...
  shape. This is what makes a manual-edit revalidation honest: the manual-edit path
...
- The Prisma row → domain entity conversion (e.g. a `staffMember` row → `StaffMember`) happens in a
  ...
  (lint-enforced), so this conversion can only happen on the service side of
...
- Prisma: `@id @default(uuid())` — never `autoincrement()`. Consistent with treating a schedule as
  potentially shared/exported later even though multi-user may be out of scope.
...
- Rules here apply to NEW code. If `scheduling-core` ever grows a second mutable class outside
  its one mutable class, that is a signal to re-read the core package's ADR before adding it — the whole
