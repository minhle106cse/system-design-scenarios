...
`application/repositories/` if no). Read that first, then name per the two cases above. Placement
is machine-checked by `npm run check:arch`; naming is not, so the cases above still need a human
eye.
...
**Rule:** `{Verb}{Noun}Service`, file `{verb}-{noun}.service.ts`, in `application/queries/` (or
`application/` when there is no query-repo) — this is what a query handler would be called if the
service HAD a bus to dispatch through.

⏸ **Not currently used in this scenario** — every read here goes through the QueryBus, so group 5
applies instead. Kept because the trigger is concrete and cheap to miss: **the moment a service
exposes a read path that the controller calls directly via DI rather than dispatching a `Query`
object**, this is its naming rule, and group 5 must NOT be stretched to cover it. Naming such a
class `...Handler` without a matching `Query` implies a bus dispatch that does not exist.

Upstream example (Cortex `search-service`): `SearchKnowledgeService` sits in
`application/queries/`, called straight from the controller, because that service deliberately has
no CommandBus/QueryBus — its only write arrives as an event, not an HTTP command. Its sibling
`IndexKnowledgeHandler` IS a `...Handler` because an event router dispatches it. The asymmetry is
the rule working, not an inconsistency.

Do **not** reach for this group to dodge naming a `Query`/`Handler` pair when a bus IS available —
that is group 5's job.
...
The failure this prevents is silent: a scenario with exactly ONE controller has no second file to
