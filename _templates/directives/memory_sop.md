<!-- TEMPLATE — copy into <scenario>/directives/ and specialize.
     SPECIALIZE: the 'When to SEARCH' table — name 4-5 real task classes for this scenario. Everything else is the routing rule, identical everywhere.
...
> is the terse operating version. Copied from `../_templates/directives/memory_sop.md` — the
> routing rule is stack-agnostic; only the "When to SEARCH" example row below is scenario-specific.
...
| **Design / spec / business intent** (what & why, schema, API) | `docs/*.md` | not directives |
...
<!-- {{FILL IN}} — replace with 4-5 real task categories for THIS scenario. Keep the shape: a task
     class on the left, which memory file(s) to check on the right. Don't leave the placeholder
     rows below in a real scenario's copy. -->

| Task | Read |
|---|---|
| Debug a `{{stack-specific error class, e.g. TS/Prisma/Jest}}` error | `errors.jsonl` + `gotchas.jsonl` |
| Design a pattern (`{{name 2-3 real architectural devices in this scenario}}`) | `architecture.jsonl` |
| Configure infra (`{{Docker/Prisma/framework bootstrap, whatever's real here}}`) | `gotchas.jsonl` |
| Refactor `{{the core package/service}}` | `architecture.jsonl` + `conventions.jsonl` |
| Write new `{{UI/API/whatever this scenario's other surface is}}` code | `conventions.jsonl` |
...
> repo's own build (`{{name 2-3 real entries — a library gotcha, an architecture decision}}`) —
> written using this exact format, not retrofitted. This discipline being genuinely maintained
> (not just scaffolded empty) is part of the AI Collaboration Narrative evidence — see
> `docs/12_ai_collaboration.md`.
