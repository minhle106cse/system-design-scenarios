...
function parseInboundTraceparent(
  ...
  // Non-null: TRACEPARENT_RE has exactly two capture groups, both guaranteed present whenever
  // `match` itself is non-null — noUncheckedIndexedAccess can't see that regex invariant statically
  // (added when this repo's tsconfig.base.json turned that flag on; not present in the source repo).
