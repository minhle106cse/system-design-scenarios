...
| 15 | **Persistence: database or browser storage?** The brief permits in-memory, local storage or a database. | **PostgreSQL via Prisma, run in Docker** (`docker-compose.yml`, Postgres only). A real schema and real migrations, behind a real database server. | A modelled schema is worth showing — it is where assumptions 3, 10 and 11 become enforceable. An earlier pass chose SQLite specifically to avoid a database server, arguing a single-user single-process app didn't need one and that a container cost more than it bought against *"prefer a one- or two-command setup."* The user overturned that reversal directly: this collection's own architecture principle is that a scenario ships a real backend service, not route handlers absorbing persistence — that principle outranks the "no container" argument for this repo. `backend-architecture-reversal.plan.md` is the record of the reversal-of-the-reversal; `docs/09_running_it.md`'s `docker compose up -d` is the one extra command it costs. |
