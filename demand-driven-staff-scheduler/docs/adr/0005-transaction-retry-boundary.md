# ADR-0005 — Transaction & Retry Boundary: Unit of Work + Inferred from Signature + Fail-Fast at Boot

> **Ported from `../../../service-appointment-scheduler/docs/adr/0001-transaction-retry-boundary.md`**
> (itself ported from Cortex), numbered **0005 here** — this repo's 0001–0004 already belong to the
> auto-scheduler algorithm ADRs (constraint enforcement, the algorithm, the demand→headcount model,
> `scheduling-core`'s zero-dependency rule). Internal section numbering (§2.3/§2.4 etc.) preserved
> exactly, because the ported source in `packages/shared-kernel` cites this ADR **by section**
> (`command-bus.ts`, `tx-scope.ts`, `cqrs.error.ts`, several interfaces) — renumbering the sections
> would point every one of those comments at the wrong place within the doc. Every in-code
> reference was updated from "ADR-0001" to "ADR-0005" as part of the port
> (`backend-architecture-reversal.plan.md` §4). Condensed in §9/§9b/§9c: the source narrates a
> migration across 5 services (Cortex) then 1 (booking); this repo has one too, so only the
> substance is kept, not a third per-service rollout log.
...
export class SchedulerApiRepoFactory implements IRepoFactory<SchedulerApiRepos, Prisma.TransactionClient> {
  create(tx: Prisma.TransactionClient): SchedulerApiRepos {
    return { schedules: new PrismaScheduleRepository(tx), assignments: new PrismaAssignmentRepository(tx) }
  }
}
```
...
- `../../../service-appointment-scheduler/docs/adr/0002-booking-concurrency-control.md` — the
  decision the source repo built on top of this boundary (booking-specific; this repo has no
  equivalent row-level concurrency constraint — see ADR-0001 here for why not)
