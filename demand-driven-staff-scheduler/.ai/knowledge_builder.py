...
def build_index() -> str:
    ...
    sections = []
    # NOTE: ported from service-appointment-scheduler/.ai/knowledge_builder.py (plan §5) with the
    # §1 project-overview text and the Turborepo/NestJS-specific framework labels swapped for this
    # scenario's shape (one Next.js app, npm workspaces, no turbo). The rest of the file — memory
    # rendering, directive/docs scanning, module auto-detection — is generic and unchanged.
...
---""")
    ...
    project_name = "Demand-Driven Staff Scheduler"
