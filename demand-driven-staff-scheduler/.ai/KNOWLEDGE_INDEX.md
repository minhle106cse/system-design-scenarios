...
> Last updated: 2026-08-21T09:04:09.643245+00:00
> 
...
Key sections:- CQRS Command Pipeline & the Unit-of-Work boundary  - Problem  - Solution: the transaction is a VALUE that owns the repositories    - 1. The write repository takes the transaction client    - 2. Reads that run outside a transaction use a READ port    - 3. The handler's TYPE declares what it needs — there is no flag    - 4. Saga = compensation stack, not atomicity (kept for the seam, not currently used)    - 5. The pipeline order is structural    - 6. Construction-time guarantee + re-entrancy guard  - Folder Structure & Clean Architecture  - Repository-interface & DTO placement — CANONICAL    - The decision procedure — answer in this ORDER, stop at the first Yes    - Remaining rules  - A command that needs to read mid-flight reads through the domain/write repo, NEVER through a query-repo
### 📄 `directives/database_standard.md` — SOP: Database & Prisma Standard
Key sections:- SOP: Database & Prisma Standard  - 🎯 Goal  - 📜 Architecture & Required Conventions    - 1. Naming Conventions    - 2. Primary Keys    - 3. Data Lifecycle (Soft Delete)    - 3.1 Cross-model "touch" writes inside the same transaction    - 4. Prisma Client Generation    - 5. Prisma version — this repo pins **5.x**, deliberately    - 6. Port Conflict — Docker Postgres
...
Key sections:- SOP: Frontend Standard  - 1. The five UI rules — 1–3 taken directly from the grading criteria (plan §3.2)  - 2. Component conventions  - 3. Styling  - 4. Talking to the API  - ⚠️ How to apply this file
...
Key sections:- SOP: Naming Conventions  - 1. `scheduling-core` stage modules  - 2. The gate and its verdict types  - 3. Reason codes  - 4. Repository (domain interface + infrastructure impl)  - 5. Command/Query Handler (CQRS)  - 6. Domain error class  - 7. NestJS Module (`@Module`)  - 8. Zod schemas  - 9. Config env var  - 10. Application Service (application-layer orchestration with no CommandBus/QueryBus)  - 11. `presentation/` controller sub-folder — always nested under `controllers/`  - ⚠️ How to apply this file
...
> **Not inlined here — see `.ai/GOTCHAS.md`** (82 entries, newest first).
