...
> The **map** of this project's knowledge system, ported from
> `../service-appointment-scheduler/.ai/KNOWLEDGE_ARCHITECTURE.md` (plan §5) with the Turborepo/
> submodule-specific mechanics already removed at the source (that repo has no `.gitmodules`
> either). `AGENTS.md` is the operating summary; this is the rationale behind it.
...
|---|---|---|
| Answers | "What is the system, why, what must it do, how is it run?" | "When I write a file, what rule must I not violate?" |
| Reader intent | Understand / operate / audit | Execute — write compliant code |
...
| `memory/*.jsonl` | Experience buffer: `errors` / `gotchas` / `architecture` / `conventions`. Local, gitignored. Surfaced in the index. | Appended by hand, After-Task |
| `knowledge_builder.py` | The generator. Scans `directives/` + `docs/` + `memory/` + `apps/*/src` + curated files. Ported with only the §1 project-overview text and framework labels swapped (plan §5). | — |
...
| Design / spec / business intent (schema, API, algorithm, why) | `docs/*.md` |
...
1. **Docs sync-trigger** — a task that changes schema / API / the algorithm's public surface MUST
   reconcile the matching `docs/NN_*.md` in the **same task** (`AGENTS.md` After-Task Protocol).
2. **The `Stop` hook** (`scripts/sync.cjs`) — regenerates the index + `GOTCHAS.md`, and **blocks the
   ...
   deliberately **not** accepted as a substitute — it is a conditional After-Task step); and
   `AGENTS.md` edited without `CLAUDE.md` in the same change — Claude Code auto-loads only the
   latter, which is why the latter duplicates rather than links.
3. **The `UserPromptSubmit` hook** (`.claude/hooks/turn-context.cjs`) — injects **turn-local
   state**: branch, uncommitted paths, outstanding After-Task debt, plus a one-line pointer at
...
4. **Citation Protocol** (`AGENTS.md`) — plans must cite which directives/docs they used.
...
- **Why this repo looks the way it does:** `.ai/plans/init-source.plan.md`.

## Not built (deliberately)

The AI workflow is intentionally lightweight: Markdown + two hooks + a Python generator, no
framework — the same "sequence, not omission" reasoning plan §1 applies to the product's own
architecture applies here too.
