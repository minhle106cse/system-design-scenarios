...
## Pre-submission defect sweep (2026-08-20) — the last six things wrong with the repo

An external review of the whole submission (running the app, not only reading it) found six real
defects. All six are fixed and verified; the suite is **289 specs green**, `typecheck`, `lint` and
`format:check` clean.

1. ⭐ **Drag-and-drop was broken for anyone near their weekly cap.** The roster grid implemented a
   move as `POST` (add) then `DELETE` (remove), so `AddAssignmentHandler` validated the destination
   against a roster that still held the source seat: H1 saw the person's hours rising by a whole
   shift and refused. Four of twelve seeded staff sit at exactly 100% utilisation, so the brief's
   own first stretch goal failed on the demo data. Fixed with `PATCH .../roster/assignments/:id`
   (`MoveAssignmentHandler`) — one operation, validated against the roster *without* the source
   seat, updating the existing row so the seat is never duplicated or lost. Verified live against
   a 24/24h staff member: the old `POST` still 422s (correctly — it is still an add), the new
   `PATCH` succeeds, and H2/H3/H4 still reject what they should.
2. **Coverage told the reader to "check the Roster tab for details"** — a tab that never showed
   them, and which no longer even held the auto-schedule button after the Schedule/Roster split.
3. **Schedule printed raw `ReasonCode` enums** (`WOULD_EXCEED_MAX_HOURS, UNAVAILABLE`) at the
   manager, breaking `frontend_standard.md` §1 rule 1 — while `error-copy.ts` already held
   human-readable copy for those exact codes. (2) and (3) are one fix: `UnfilledSeatsBanner`,
   rendered identically on both screens, naming day, shift and reason.
4. **The CI workflow could never run.** It filtered on `paths: ['demand-driven-staff-scheduler/**']`
   and set a matching `working-directory` — neither of which exists in the `git subtree split`
   repository that is actually submitted, and GitHub reads `.github/workflows/` only at a repo
   root, so it was inert in the collection repo too. Now root-relative, with `format:check` added.
5. **No error boundary.** `npm run dev` starts both apps in parallel; the API boots slower, so
   opening the UI first showed Next's unhandled-error screen. `app/error.tsx` now explains the
   likely cause. Its retry is `window.location.reload()` — `reset()` and `router.refresh()`
   + `reset()` were both tried and **neither recovered**, verified by stopping and restarting the
   API against a live browser.
6. **Documentation drift, twice.** `docs/08` claimed Layer 3 was "Jest + a real Postgres" covering
   "Controllers" — every spec drives mocked repositories and no controller is tested anywhere; and
   the spec counts (255/97/64/41) had drifted 34 specs behind the suite. Both corrected against a
   forced test run.

Setup also went from five commands to two (`npm install && npm run setup`, then `npm run dev`) —
...
- **No HTTP-level test.** Every `apps/scheduler-api` spec mocks its repositories; no test drives a
  controller, a route, or a serialized response, and nothing runs against a real Postgres. The
  contract is verified by hand and by `apps/web` calling it. Closing this means supertest against
  a real database, the way scenario 01 does it. Stated here, and in `docs/08`, rather than left
  looking covered.
