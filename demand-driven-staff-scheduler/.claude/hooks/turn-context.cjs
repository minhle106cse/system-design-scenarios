 ...
 * has no `.gitmodules` (single repo, single app, see .ai/plans/init-source.plan.md §0/§5). Cortex's
 * version parses `.gitmodules` and runs `git status` inside each submodule because its root `git
 * status` only reports a submodule POINTER, never the files inside. That problem doesn't exist
 * here: `apps/web` and `packages/scheduling-core` are ordinary tracked directories, so plain
 * `git status --short` at the root already sees every changed file.
 *
