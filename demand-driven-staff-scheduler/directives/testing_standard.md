...
> CommonJS). This repo uses **Vitest**, which needs none of that (plan §0). What's genuinely new
> here is §2 — **property-based testing**, this repo's flagship layer (plan §8.1) — which the
...
## 📜 The three layers (plan §8) — pick by what the test needs to prove
...
- **Two runners, by package** — `packages/scheduling-core` and `apps/web` use **Vitest**;
  `packages/shared-kernel` and `apps/scheduler-api` use **Jest**, matching the toolchain each was
  ported with (`backend-architecture-reversal.plan.md` §6 — a green 80-test suite was not migrated
  for consistency alone). `npm test` at the root fans out to all four through Turborepo.
- Both suffixes run on `npm test` (`vitest.config.ts`'s `include` lists both). Layers 1–2 need no
  infrastructure (plan §2.2 — `scheduling-core` runs in milliseconds); **layer 3 needs Postgres
  up** (`docker compose up -d`), which is why a change to `apps/scheduler-api` is verified against
...
- **Totality** — `generateRoster` never throws for any input the arbitrary can produce (a thrown
  error here is *always* a bug: the brief requires graceful degradation, plan §7.6).
...
`scheduling-core` has no dependencies to mock (plan §2.1) — its tests call the real functions. When
...
In `apps/web` (Vitest) the same pattern uses `vi.fn()` and `Mocked<T>` from `vitest`. This avoids
...
`apps/web` maps it in `tsconfig.json` + `vitest.config.ts`; `apps/scheduler-api` maps it in
