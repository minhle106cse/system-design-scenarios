...
> (assumption 11), `POST .../demand/import` upserts on `(scheduleId, dayOfWeek, hour)`
> (assumption 10). Running either twice yields the same state, so a double-submitted form cannot
...
`docs/adr/0005-transaction-retry-boundary.md` — ADR-**0005** here, 0001 in scenario 01):
...
- `docs/adr/0005-transaction-retry-boundary.md` — the transaction/retry boundary an
