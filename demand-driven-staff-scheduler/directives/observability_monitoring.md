...
> (`backend-architecture-reversal.plan.md` §6 — Prometheus/Grafana were not ported, trigger below).
...
> containers. **Trigger to build this out** (`backend-architecture-reversal.plan.md` §6): an
...
Prometheus would scrape it via `host.docker.internal:4102` — **note the port: this repo's API is
`4102`**, not scenario 01's `4002`. A red target means the app isn't running on the host, not a
Prometheus misconfiguration. ⚠️ Grafana's default `3000` collides with `apps/web`'s dev server
here — pick a different `GRAFANA_PORT` when wiring this, a conflict scenario 01 doesn't have
because it ships no frontend.
