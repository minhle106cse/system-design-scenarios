...
- `scheduling-core` in particular **must never validate** — it has no Zod dependency by design
  (plan §2.1) and trusts its caller completely; that trust boundary is `apps/scheduler-api`'s job
  to hold.
- The `FeasibilityGate` (plan §7.4) is not a validation layer in this sense — it enforces *domain*
...
3. Verify a malformed request returns a 400 with a field-precise message, by hand or via a test —
   the same discipline the CSV importer owes the manager (plan §4, business-requirements.md #9):
   never a bare 500.
