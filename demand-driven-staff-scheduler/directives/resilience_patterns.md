...
construction — `POST .../auto-schedule` is a full replace (assumption 11), `POST .../demand/import`
upserts on `(scheduleId, dayOfWeek, hour)` (assumption 10). Running either twice yields the same
...
synchronous side effect of the same request — here that would be something like alerting the
manager when auto-schedule can't cover a shift. See `docs/03_architecture.md § Deferred scope`.
...
when the `FeasibilityGate` refuses the candidate assignment (H1–H3, ADR-0001). That error is
