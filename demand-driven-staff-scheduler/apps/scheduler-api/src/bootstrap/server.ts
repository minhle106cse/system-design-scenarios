...
export async function buildServer() {
  const adapter = new FastifyAdapter({
    ...
    genReqId: (req: IncomingMessage) => {
      ...
      // Array.isArray branch: noUncheckedIndexedAccess (this repo's tsconfig.base.json, not the
      // source repo's) types header[0] as `string | undefined` even though the array itself is
      // `string[]` — `?? randomUUID()` on both branches keeps the return type `string`.
