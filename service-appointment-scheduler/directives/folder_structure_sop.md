...
>
> Ported from Cortex's `folder_structure_sop.md`, translated to English and trimmed to this
> repo's actual shape: **NestJS only** (no hand-rolled Fastify `container/` — NestJS's own DI
> module system replaces the manual wiring Cortex's Fastify-native `auth-service` needed), and
> **one service** (no core-api-vs-auth-service comparison section).
...
│   ├── fastify.ts                   # Fastify instance + plugin registration (cors/helmet/compress)
...
│   ├── http/
│   │   ├── controllers/             # health.controller.ts — infra endpoints only
...
| `modules/` | Business logic by domain | `common/`, `packages/shared-kernel`, `infrastructure/cqrs` (application layer only) |
...
| `modules/*/application/**` | ORM/DB/HTTP infra; **HTTP exceptions** (`NotFoundException`, …) from `@nestjs/common` | repository interfaces; `@/infrastructure/cqrs` (decorators); NestJS DI (`@Injectable`/`@Inject`) |
...
4. Does the file belong to a specific **feature domain**?
   - → `modules/<domain>/`
5. Run `npm run lint` before committing — the boundary rules in §Enforcement will block a
