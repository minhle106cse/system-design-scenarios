...
> Heavily trimmed from Cortex's 345-line version, which spends most of its length on
> Elasticsearch index routing, Kibana RBAC, ILM retention, gRPC boundary logging, and Kafka
> consumer logging — none of that exists here (no ES, no gRPC, no message consumer at T1/T2, see
> `.ai/plans/init-source.plan.md` §4/§7). What's kept is the part that's actually built and shipped in this repo:
> the dual-logging philosophy, the logger-hierarchy rule, explicit `LogContext`, redaction, and
> the two real bugs Cortex found and fixed — both fixes are already baked into the ported code.
